...
import MuteAudioToolbar from './muteAudioToolbar';
...
class HallOfBotanyView extends React.Component {
  ...
  componentDidMount() {
    const notificationText = 'Do you hear that? Something is playing faintly in ' +
                             'the background, you should open the Hall Of Botany ' +
                             'app and hear it yourself.';
    ...
    NativeAppEventEmitter.addListener('BeaconManagerBeaconPing', (body) =>
      this.beaconNotificationPing(body)
    );
  ...
  beaconNotificationPing(body) {
    const { loadAudio, playAudio, adjustAudioVolume, stopAudio,
            changeActiveBeacon, clearActiveBeacon, updateActiveBeacon,
            activeBeacon } = this.props;
  ...
  render() {
    const { activeBeacon, audio, toggleMuteAudio } = this.props;
    const debug = false;
    let body;
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={{ opacity: debug ? 1.0 : 0.0 }}>
          <View style={styles.separator} />
          <View style={styles.toolbar}>
        ...
        <MuteAudioToolbar muted={audio.muted} toggleMuteAudio={toggleMuteAudio}/>
      </View>
    );
  }
}
